(function(c){c.widget("blueimp.imagegallery",{options:{namespace:"imagegallery",selector:"a[rel=gallery]",modalSelector:"#gallery-modal",loaderSelector:"#gallery-loader",backdrop:!0,keyboard:!0,offsetWidth:100,offsetHeight:200,canvas:!1,slideshow:0},_load:function(b){var a=b.data.gallery,d=a.options,e=c(window).width(),f=c(window).height(),g=a._modal.hasClass("fullscreen");b.preventDefault();a._abortHandler();window.clearTimeout(a._slideShow);a.defer=c.Deferred();a._isModalLink=c(this).parent().hasClass("modal-footer");
a._isModalLink?(a._currentLink=c(this).hasClass("next")?c(a._currentLink).data("next"):c(a._currentLink).data("prev"),a._modal.removeClass("in")):(a._initLinks(),a._currentLink=this,a._initAbortHandlers());a._isModalLink&&a._transition?a._modal.bind(a._transitionEnd+"."+d.namespace,function(b){b.target===a._modal[0]&&(a._modal.unbind(a._transitionEnd+"."+d.namespace),a._initModal(),a.defer.resolve())}):(a._modal.hide(),a._initModal(),a.defer.resolve());b=g?{minWidth:e,minHeight:f,maxWidth:e,maxHeight:f,
canvas:d.canvas}:{maxWidth:e-d.offsetWidth,maxHeight:f-d.offsetHeight,canvas:d.canvas};a._loadingImageTimeout=window.setTimeout(function(){a._loader.addClass("in")},100);a._loadingImage=window.loadImage(a._currentLink.href,function(b){a._loadHandler(b)},b);a._preload()},_loadHandler:function(b){var a=this;this._abortHandler();this._isModalLink?this.defer.done(function(){a._modal.show();a._modal.addClass("in");a._initModalBody(b)}):(this._initModalBody(b),this._modal.modal("show"));if(this.options.slideshow)this._slideShow=
window.setTimeout(function(){a._next()},this.options.slideshow)},_initLinks:function(){var b=c(this.element).find(this.options.selector);b.each(function(a,d){var e=b[a-1],f=b[a+1];if(!e||e.href===d.href)if(e=b[a-2],!e||e.href===d.href)e=b[b.length-1];if(!f||f.href===d.href)if(f=b[a+2],!f||f.href===d.href)f=b[0];c(d).data("prev",e);c(d).data("next",f)})},_initModal:function(){var b=c(this._currentLink).data("prev"),a=c(this._currentLink).data("next");this._modalTitle.text(this._currentLink.title);
...
c(this.element).undelegate(this.options.selector,"click."+this.options.namespace)},_initTransitionSupport:function(){var b=this;c(function(){b._transition=c.support.transition&&b._modal.hasClass("fade");if(c.support.transition)if(b._transitionEnd="TransitionEnd",c.browser.webkit)b._transitionEnd="webkitTransitionEnd";else if(c.browser.mozilla)b._transitionEnd="transitionend";else if(c.browser.opera)b._transitionEnd="oTransitionEnd"})},_create:function(){this._modal=c(this.options.modalSelector).modal({backdrop:this.options.backdrop,
keyboard:this.options.keyboard});this._loader=c(this.options.loaderSelector);this._modalTitle=this._modal.find(".modal-header .title");this._modalBody=this._modal.find(".modal-body");this._modalDownload=this._modal.find(".modal-footer .download");this._modalPrev=this._modal.find(".modal-footer .prev");this._modalNext=this._modal.find(".modal-footer .next");this._initTransitionSupport();this._initEventHandlers()},destroy:function(){window.clearTimeout(this._slideShow);this._destroyEventHandlers();
